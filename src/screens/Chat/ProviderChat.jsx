...
function ProviderChat() {
    ...
    useEffect(() => {
        axiosInstance.get(`/user/providerChat/${providerId}`).then((res) => {
    ...
    const addMessage = async () => {
        try {
            if (message.trim().length !== 0) {
                await axiosInstance.post('/user/addmessage', { message, chatId, userId })
                setMessage('')
                setLoading(true)
            }
                            ...
                            <div className="h-full relative">
                                ...
                                {/* Users */}
                                {
                                    chats.map((chat, i) => {
                                        ...
                                        return (
                                            <div key={i} className="relative rounded-lg px-2 py-2 flex items-start space-x-3 hover:border-gray-400 focus-within:ring-2 mb-3 bg-gray-200">
                                                <div className="flex-shrink-0">
